...

{
  options = {
    services.etherpad = mkOption {
      type = types.attrsOf (types.submodule (
        {
          options = {
            ...
            name = mkOption {
              default = "${config.networking.hostName}-etherpad";
              description = ''
                Specifies the Name of the Etherpad daemon.
              '';
            };
            title = mkOption {
              default = "Etherpad";
            ...
            extraConfig = mkOption {
              default = "";
              description = "
                Extra text to be appended to the settings.json server configuration
                file.  
              ";
            };
          };
      ...
      '';
      description = "etherpad lite instance ‘’";
    };
  };

  config = {
    users.extraUsers.etherpad = { 
      description = "etherpad lite user";
      name = "etherpad";
      home = "/var/empty";
    };

    systemd.services."etherpad@" = { 
      description = "Etherpad service '%i'";

      after = [ "network.target" ];
      wantedBy = [ "multi-user.target" ];

      environment.INSTANCE = "%i";
      environment.root = "${pkgs.etherpad}";

      script = "exec ${pkgs.nodejs}/bin/node ${pkgs.etherpad}/node_modules/ep_etherpad-lite/node/server.js --settings /etc/etherpad/settings-foo.json";
